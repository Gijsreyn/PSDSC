...
Describe 'TestPsPathExtension' {
    Context 'When file paths are correct' {
        ...
        It 'Should return PowerShell script file is true' {
            InModuleScope -ScriptBlock {
                $result = TestPsPathExtension -Path (Join-Path -Path $TestDrive -ChildPath 'test.ps1')
    ...
    Context 'When file paths are incorrect' {
        ...
        It 'Should return PowerShell script file is false because it is a PowerShell module file' {
            InModuleScope -ScriptBlock {
                $result = TestPsPathExtension -Path (Join-Path -Path $TestDrive -ChildPath 'test.psm1')
        ...
        It 'Should return false because file path does not exist' {
            InModuleScope -ScriptBlock {
                $result = TestPsPathExtension -Path (Join-Path -Path $TestDrive -ChildPath 'thisdoesnotexist.txt')
